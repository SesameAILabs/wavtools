(()=>{var d=class{static floatTo16BitPCM(e){let a=new ArrayBuffer(e.length*2),t=new DataView(a),r=0;for(let s=0;s<e.length;s++,r+=2){let n=Math.max(-1,Math.min(1,e[s]));t.setInt16(r,n<0?n*32768:n*32767,!0)}return a}static mergeBuffers(e,a){let t=new Uint8Array(e.byteLength+a.byteLength);return t.set(new Uint8Array(e),0),t.set(new Uint8Array(a),e.byteLength),t.buffer}_packData(e,a){return[new Uint8Array([a,a>>8]),new Uint8Array([a,a>>8,a>>16,a>>24])][e]}pack(e,a){if(a?.bitsPerSample)if(a?.channels){if(!a?.data)throw new Error('Missing "data"')}else throw new Error('Missing "channels"');else throw new Error('Missing "bitsPerSample"');let{bitsPerSample:t,channels:r,data:s}=a,n=["RIFF",this._packData(1,52),"WAVE","fmt ",this._packData(1,16),this._packData(0,1),this._packData(0,r.length),this._packData(1,e),this._packData(1,e*r.length*t/8),this._packData(0,r.length*t/8),this._packData(0,t),"data",this._packData(1,r[0].length*r.length*t/8),s],i=new Blob(n,{type:"audio/mpeg"}),l=URL.createObjectURL(i);return{blob:i,url:l,channelCount:r.length,sampleRate:e,duration:s.byteLength/(r.length*e*2)}}};globalThis.WavPacker=d;var B=[4186.01,4434.92,4698.63,4978.03,5274.04,5587.65,5919.91,6271.93,6644.88,7040,7458.62,7902.13],_=["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"],m=[],S=[];for(let f=1;f<=8;f++)for(let e=0;e<B.length;e++){let a=B[e];m.push(a/Math.pow(2,8-f)),S.push(_[e]+f)}var v=[32,2e3],R=m.filter((f,e)=>m[e]>v[0]&&m[e]<v[1]),M=S.filter((f,e)=>m[e]>v[0]&&m[e]<v[1]);var g=class f{static getFrequencies(e,a,t,r="frequency",s=-100,n=-30){t||(t=new Float32Array(e.frequencyBinCount),e.getFloatFrequencyData(t));let i=a/2,l=1/t.length*i,u,o,c;if(r==="music"||r==="voice"){let p=r==="voice"?R:m,h=Array(p.length).fill(s);for(let w=0;w<t.length;w++){let D=w*l,P=t[w];for(let k=p.length-1;k>=0;k--)if(D>p[k]){h[k]=Math.max(h[k],P);break}}u=h,o=r==="voice"?R:m,c=r==="voice"?M:S}else u=Array.from(t),o=u.map((p,h)=>l*h),c=o.map(p=>`${p.toFixed(2)} Hz`);let y=u.map(p=>Math.max(0,Math.min((p-s)/(n-s),1)));return{values:new Float32Array(y),frequencies:o,labels:c}}constructor(e,a=null){if(this.fftResults=[],a){let{length:t,sampleRate:r}=a,s=new OfflineAudioContext({length:t,sampleRate:r}),n=s.createBufferSource();n.buffer=a;let i=s.createAnalyser();i.fftSize=8192,i.smoothingTimeConstant=.1,n.connect(i);let l=1/60,u=t/r,o=c=>{let y=l*c;y<u&&s.suspend(y).then(()=>{let b=new Float32Array(i.frequencyBinCount);i.getFloatFrequencyData(b),this.fftResults.push(b),o(c+1)}),c===1?s.startRendering():s.resume()};n.start(0),o(1),this.audio=e,this.context=s,this.analyser=i,this.sampleRate=r,this.audioBuffer=a}else{let t=new AudioContext,r=t.createMediaElementSource(e),s=t.createAnalyser();s.fftSize=8192,s.smoothingTimeConstant=.1,r.connect(s),s.connect(t.destination),this.audio=e,this.context=t,this.analyser=s,this.sampleRate=this.context.sampleRate,this.audioBuffer=null}}getFrequencies(e="frequency",a=-100,t=-30){let r=null;if(this.audioBuffer&&this.fftResults.length){let s=this.audio.currentTime/this.audio.duration,n=Math.min(s*this.fftResults.length|0,this.fftResults.length-1);r=this.fftResults[n]}return f.getFrequencies(this.analyser,this.sampleRate,r,e,a,t)}async resumeIfSuspended(){return this.context.state==="suspended"&&await this.context.resume(),!0}};globalThis.AudioAnalysis=g;var F=`
// StreamProcessor
...
`,q=new Blob([F],{type:"application/javascript"}),T=URL.createObjectURL(q),C=T;var A=class{constructor({sampleRate:e=44100}={}){this.scriptSrc=C,this.sampleRate=e,this.context=null,this.stream=null,this.analyser=null,this.trackSampleOffsets={},this.interruptedTrackIds={},this._audioProcessor=()=>{}}async connect(e){this.context=new AudioContext({sampleRate:this.sampleRate}),this._audioProcessor=e,this.context.state==="suspended"&&await this.context.resume();try{await this.context.audioWorklet.addModule(this.scriptSrc)}catch(t){throw console.error(t),new Error(`Could not add audioWorklet module: ${this.scriptSrc}`)}let a=this.context.createAnalyser();return a.fftSize=8192,a.smoothingTimeConstant=.1,this.analyser=a,!0}getFrequencies(e="frequency",a=-100,t=-30){if(!this.analyser)throw new Error("Not connected, please call .connect() first");return g.getFrequencies(this.analyser,this.sampleRate,null,e,a,t)}_start(){let e=new AudioWorkletNode(this.context,"stream_processor");return e.connect(this.context.destination),e.port.onmessage=a=>{let{event:t,data:r,underrun:s,timestamp_ms:n}=a.data;if(t==="audio")this._audioProcessor(r,s,n);else if(t==="stop")e.disconnect(),this.stream=null;else if(t==="offset"){let{requestId:i,trackId:l,offset:u}=a.data,o=u/this.sampleRate;this.trackSampleOffsets[i]={trackId:l,offset:u,currentTime:o}}else t==="log"&&console.log(r)},this.analyser.disconnect(),e.connect(this.analyser),this.stream=e,!0}configure(e){this.stream.port.postMessage({event:"configure",config:e})}add16BitPCM(e,a="default"){if(typeof a!="string")throw new Error("trackId must be a string");if(this.interruptedTrackIds[a])return;this.stream||this._start();let t;if(e instanceof Int16Array)t=e;else if(e instanceof ArrayBuffer)t=new Int16Array(e);else throw new Error("argument must be Int16Array or ArrayBuffer");return this.stream.port.postMessage({event:"write",buffer:t,trackId:a}),t}async getTrackSampleOffset(e=!1){if(!this.stream)return null;let a=crypto.randomUUID();this.stream.port.postMessage({event:e?"interrupt":"offset",requestId:a});let t;for(;!t;)t=this.trackSampleOffsets[a],await new Promise(s=>setTimeout(()=>s(),1));let{trackId:r}=t;return e&&r&&(this.interruptedTrackIds[r]=!0),t}async interrupt(){return this.getTrackSampleOffset(!0)}};globalThis.WavStreamPlayer=A;var E=`
...
`,L=new Blob([E],{type:"application/javascript"}),O=URL.createObjectURL(L),I=O;var x=class{constructor({sampleRate:e=44100,outputToSpeakers:a=!1,debug:t=!1}={}){this.scriptSrc=I,this.sampleRate=e,this.outputToSpeakers=a,this.debug=!!t,this._deviceChangeCallback=null,this._devices=[],this.stream=null,this.processor=null,this.source=null,this.node=null,this.recording=!1,this._lastEventId=0,this.eventReceipts={},this.eventTimeout=5e3,this._chunkProcessor=()=>{},this._chunkProcessorSize=void 0,this._chunkProcessorBuffer={raw:new ArrayBuffer(0),mono:new ArrayBuffer(0)}}static async decode(e,a=44100,t=-1){let r=new AudioContext({sampleRate:a}),s,n;if(e instanceof Blob){if(t!==-1)throw new Error('Can not specify "fromSampleRate" when reading from Blob');n=e,s=await n.arrayBuffer()}else if(e instanceof ArrayBuffer){if(t!==-1)throw new Error('Can not specify "fromSampleRate" when reading from ArrayBuffer');s=e,n=new Blob([s],{type:"audio/wav"})}else{let o,c;if(e instanceof Int16Array){c=e,o=new Float32Array(e.length);for(let h=0;h<e.length;h++)o[h]=e[h]/32768}else if(e instanceof Float32Array)o=e;else if(e instanceof Array)o=new Float32Array(e);else throw new Error('"audioData" must be one of: Blob, Float32Arrray, Int16Array, ArrayBuffer, Array<number>');if(t===-1)throw new Error('Must specify "fromSampleRate" when reading from Float32Array, In16Array or Array');if(t<3e3)throw new Error('Minimum "fromSampleRate" is 3000 (3kHz)');c||(c=d.floatTo16BitPCM(o));let y={bitsPerSample:16,channels:[o],data:c};n=new d().pack(t,y).blob,s=await n.arrayBuffer()}let i=await r.decodeAudioData(s),l=i.getChannelData(0),u=URL.createObjectURL(n);return{blob:n,url:u,values:l,audioBuffer:i}}log(){return this.debug&&this.log(...arguments),!0}getSampleRate(){return this.sampleRate}getStatus(){return this.processor?this.recording?"recording":"paused":"ended"}async _event(e,a={},t=null){if(t=t||this.processor,!t)throw new Error("Can not send events without recording first");let r={event:e,id:this._lastEventId++,data:a};t.port.postMessage(r);let s=new Date().valueOf();for(;!this.eventReceipts[r.id];){if(new Date().valueOf()-s>this.eventTimeout)throw new Error(`Timeout waiting for "${e}" event`);await new Promise(i=>setTimeout(()=>i(!0),1))}let n=this.eventReceipts[r.id];return delete this.eventReceipts[r.id],n}listenForDeviceChange(e){if(e===null&&this._deviceChangeCallback)navigator.mediaDevices.removeEventListener("devicechange",this._deviceChangeCallback),this._deviceChangeCallback=null;else if(e!==null){let a=0,t=[],r=n=>n.map(i=>i.deviceId).sort().join(","),s=async()=>{let n=++a,i=await this.listDevices();n===a&&r(t)!==r(i)&&(t=i,e(i.slice()))};navigator.mediaDevices.addEventListener("devicechange",s),s(),this._deviceChangeCallback=s}return!0}async requestPermission(){let e=await navigator.permissions.query({name:"microphone"});if(e.state==="denied")window.alert("You must grant microphone access to use this feature.");else if(e.state==="prompt")try{(await navigator.mediaDevices.getUserMedia({audio:!0})).getTracks().forEach(r=>r.stop())}catch{window.alert("You must grant microphone access to use this feature.")}return!0}async listDevices(){if(!navigator.mediaDevices||!("enumerateDevices"in navigator.mediaDevices))throw new Error("Could not request user devices");await this.requestPermission();let a=(await navigator.mediaDevices.enumerateDevices()).filter(s=>s.kind==="audioinput"),t=a.findIndex(s=>s.deviceId==="default"),r=[];if(t!==-1){let s=a.splice(t,1)[0],n=a.findIndex(i=>i.groupId===s.groupId);n!==-1&&(s=a.splice(n,1)[0]),s.default=!0,r.push(s)}return r.concat(a)}async begin(e){if(this.processor)throw new Error("Already connected: please call .end() to start a new session");if(!navigator.mediaDevices||!("getUserMedia"in navigator.mediaDevices))throw new Error("Could not request user media");try{let i={audio:{echoCancellation:!0}};e&&(i.audio.deviceId={exact:e}),this.stream=await navigator.mediaDevices.getUserMedia(i);let l=this.stream.getAudioTracks()[0];console.log("track-settings",l.getSettings())}catch{throw new Error("Could not start media stream")}let a=new AudioContext({sampleRate:this.sampleRate}),t=a.createMediaStreamSource(this.stream);try{await a.audioWorklet.addModule(this.scriptSrc)}catch(i){throw console.error(i),new Error(`Could not add audioWorklet module: ${this.scriptSrc}`)}let r=new AudioWorkletNode(a,"audio_processor");r.port.onmessage=i=>{let{event:l,id:u,data:o,timestamp_ms:c}=i.data;if(l==="receipt")this.eventReceipts[u]=o;else if(l==="chunk"){if(this._chunkProcessorSize)throw new Error("deprecated - chunkSize must be 0 - do not use buffering");this._chunkProcessor(o,c)}};let s=t.connect(r),n=a.createAnalyser();return n.fftSize=8192,n.smoothingTimeConstant=.1,s.connect(n),this.outputToSpeakers&&(console.warn(`Warning: Output to speakers may affect sound quality,
especially due to system audio feedback preventative measures.
use only for debugging`),n.connect(a.destination)),this.source=t,this.node=s,this.analyser=n,this.processor=r,!0}getFrequencies(e="frequency",a=-100,t=-30){if(!this.processor)throw new Error("Session ended: please call .begin() first");return g.getFrequencies(this.analyser,this.sampleRate,null,e,a,t)}async pause(){if(this.processor){if(!this.recording)throw new Error("Already paused: please call .record() first")}else throw new Error("Session ended: please call .begin() first");return this.log("Pausing ..."),await this._event("stop"),this.recording=!1,!0}async record(e=()=>{}){if(this.processor){if(this.recording)throw new Error("Already recording: please call .pause() first");if(typeof e!="function")throw new Error("chunkProcessor must be a function")}else throw new Error("Session ended: please call .begin() first");return this._chunkProcessor=e,this._chunkProcessorSize=0,this._chunkProcessorBuffer={raw:new ArrayBuffer(0),mono:new ArrayBuffer(0)},this.log("Recording ..."),await this._event("start"),this.recording=!0,!0}async clear(){if(!this.processor)throw new Error("Session ended: please call .begin() first");return await this._event("clear"),!0}async read(){if(!this.processor)throw new Error("Session ended: please call .begin() first");return this.log("Reading ..."),await this._event("read")}async save(e=!1){if(!this.processor)throw new Error("Session ended: please call .begin() first");if(!e&&this.recording)throw new Error("Currently recording: please call .pause() first, or call .save(true) to force");this.log("Exporting ...");let a=await this._event("export");return new d().pack(this.sampleRate,a.audio)}async end(){if(!this.processor)throw new Error("Session ended: please call .begin() first");let e=this.processor;this.log("Stopping ..."),await this._event("stop"),this.recording=!1,this.stream.getTracks().forEach(n=>n.stop()),this.log("Exporting ...");let t=await this._event("export",{},e);return this.processor.disconnect(),this.source.disconnect(),this.node.disconnect(),this.analyser.disconnect(),this.stream=null,this.processor=null,this.source=null,this.node=null,new d().pack(this.sampleRate,t.audio)}async quit(){return this.listenForDeviceChange(null),this.processor&&await this.end(),!0}};globalThis.WavRecorder=x;})();
