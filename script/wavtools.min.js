(()=>{var d=class{static floatTo16BitPCM(e){let s=new ArrayBuffer(e.length*2),t=new DataView(s),r=0;for(let a=0;a<e.length;a++,r+=2){let n=Math.max(-1,Math.min(1,e[a]));t.setInt16(r,n<0?n*32768:n*32767,!0)}return s}static mergeBuffers(e,s){let t=new Uint8Array(e.byteLength+s.byteLength);return t.set(new Uint8Array(e),0),t.set(new Uint8Array(s),e.byteLength),t.buffer}_packData(e,s){return[new Uint8Array([s,s>>8]),new Uint8Array([s,s>>8,s>>16,s>>24])][e]}pack(e,s){if(s?.bitsPerSample)if(s?.channels){if(!s?.data)throw new Error('Missing "data"')}else throw new Error('Missing "channels"');else throw new Error('Missing "bitsPerSample"');let{bitsPerSample:t,channels:r,data:a}=s,n=["RIFF",this._packData(1,52),"WAVE","fmt ",this._packData(1,16),this._packData(0,1),this._packData(0,r.length),this._packData(1,e),this._packData(1,e*r.length*t/8),this._packData(0,r.length*t/8),this._packData(0,t),"data",this._packData(1,r[0].length*r.length*t/8),a],i=new Blob(n,{type:"audio/mpeg"}),l=URL.createObjectURL(i);return{blob:i,url:l,channelCount:r.length,sampleRate:e,duration:a.byteLength/(r.length*e*2)}}};globalThis.WavPacker=d;var B=[4186.01,4434.92,4698.63,4978.03,5274.04,5587.65,5919.91,6271.93,6644.88,7040,7458.62,7902.13],_=["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"],m=[],S=[];for(let f=1;f<=8;f++)for(let e=0;e<B.length;e++){let s=B[e];m.push(s/Math.pow(2,8-f)),S.push(_[e]+f)}var v=[32,2e3],R=m.filter((f,e)=>m[e]>v[0]&&m[e]<v[1]),M=S.filter((f,e)=>m[e]>v[0]&&m[e]<v[1]);var g=class f{static getFrequencies(e,s,t,r="frequency",a=-100,n=-30){t||(t=new Float32Array(e.frequencyBinCount),e.getFloatFrequencyData(t));let i=s/2,l=1/t.length*i,u,o,c;if(r==="music"||r==="voice"){let p=r==="voice"?R:m,h=Array(p.length).fill(a);for(let w=0;w<t.length;w++){let D=w*l,P=t[w];for(let k=p.length-1;k>=0;k--)if(D>p[k]){h[k]=Math.max(h[k],P);break}}u=h,o=r==="voice"?R:m,c=r==="voice"?M:S}else u=Array.from(t),o=u.map((p,h)=>l*h),c=o.map(p=>`${p.toFixed(2)} Hz`);let y=u.map(p=>Math.max(0,Math.min((p-a)/(n-a),1)));return{values:new Float32Array(y),frequencies:o,labels:c}}constructor(e,s=null){if(this.fftResults=[],s){let{length:t,sampleRate:r}=s,a=new OfflineAudioContext({length:t,sampleRate:r}),n=a.createBufferSource();n.buffer=s;let i=a.createAnalyser();i.fftSize=8192,i.smoothingTimeConstant=.1,n.connect(i);let l=1/60,u=t/r,o=c=>{let y=l*c;y<u&&a.suspend(y).then(()=>{let b=new Float32Array(i.frequencyBinCount);i.getFloatFrequencyData(b),this.fftResults.push(b),o(c+1)}),c===1?a.startRendering():a.resume()};n.start(0),o(1),this.audio=e,this.context=a,this.analyser=i,this.sampleRate=r,this.audioBuffer=s}else{let t=new AudioContext,r=t.createMediaElementSource(e),a=t.createAnalyser();a.fftSize=8192,a.smoothingTimeConstant=.1,r.connect(a),a.connect(t.destination),this.audio=e,this.context=t,this.analyser=a,this.sampleRate=this.context.sampleRate,this.audioBuffer=null}}getFrequencies(e="frequency",s=-100,t=-30){let r=null;if(this.audioBuffer&&this.fftResults.length){let a=this.audio.currentTime/this.audio.duration,n=Math.min(a*this.fftResults.length|0,this.fftResults.length-1);r=this.fftResults[n]}return f.getFrequencies(this.analyser,this.sampleRate,r,e,s,t)}async resumeIfSuspended(){return this.context.state==="suspended"&&await this.context.resume(),!0}};globalThis.AudioAnalysis=g;var F=`
// StreamProcessor
