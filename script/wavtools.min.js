(()=>{var m=class{static floatTo16BitPCM(e){let a=new ArrayBuffer(e.length*2),t=new DataView(a),s=0;for(let r=0;r<e.length;r++,s+=2){let n=Math.max(-1,Math.min(1,e[r]));t.setInt16(s,n<0?n*32768:n*32767,!0)}return a}static mergeBuffers(e,a){let t=new Uint8Array(e.byteLength+a.byteLength);return t.set(new Uint8Array(e),0),t.set(new Uint8Array(a),e.byteLength),t.buffer}_packData(e,a){return[new Uint8Array([a,a>>8]),new Uint8Array([a,a>>8,a>>16,a>>24])][e]}pack(e,a){if(a?.bitsPerSample)if(a?.channels){if(!a?.data)throw new Error('Missing "data"')}else throw new Error('Missing "channels"');else throw new Error('Missing "bitsPerSample"');let{bitsPerSample:t,channels:s,data:r}=a,n=["RIFF",this._packData(1,52),"WAVE","fmt ",this._packData(1,16),this._packData(0,1),this._packData(0,s.length),this._packData(1,e),this._packData(1,e*s.length*t/8),this._packData(0,s.length*t/8),this._packData(0,t),"data",this._packData(1,s[0].length*s.length*t/8),r],i=new Blob(n,{type:"audio/mpeg"}),l=URL.createObjectURL(i);return{blob:i,url:l,channelCount:s.length,sampleRate:e,duration:r.byteLength/(s.length*e*2)}}};globalThis.WavPacker=m;var C=[4186.01,4434.92,4698.63,4978.03,5274.04,5587.65,5919.91,6271.93,6644.88,7040,7458.62,7902.13],_=["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"],d=[],A=[];for(let f=1;f<=8;f++)for(let e=0;e<C.length;e++){let a=C[e];d.push(a/Math.pow(2,8-f)),A.push(_[e]+f)}var v=[32,2e3],B=d.filter((f,e)=>d[e]>v[0]&&d[e]<v[1]),R=A.filter((f,e)=>d[e]>v[0]&&d[e]<v[1]);var g=class f{static getFrequencies(e,a,t,s="frequency",r=-100,n=-30){t||(t=new Float32Array(e.frequencyBinCount),e.getFloatFrequencyData(t));let i=a/2,l=1/t.length*i,u,o,c;if(s==="music"||s==="voice"){let p=s==="voice"?B:d,h=Array(p.length).fill(r);for(let w=0;w<t.length;w++){let I=w*l,P=t[w];for(let k=p.length-1;k>=0;k--)if(I>p[k]){h[k]=Math.max(h[k],P);break}}u=h,o=s==="voice"?B:d,c=s==="voice"?R:A}else u=Array.from(t),o=u.map((p,h)=>l*h),c=o.map(p=>`${p.toFixed(2)} Hz`);let y=u.map(p=>Math.max(0,Math.min((p-r)/(n-r),1)));return{values:new Float32Array(y),frequencies:o,labels:c}}constructor(e,a=null){if(this.fftResults=[],a){let{length:t,sampleRate:s}=a,r=new OfflineAudioContext({length:t,sampleRate:s}),n=r.createBufferSource();n.buffer=a;let i=r.createAnalyser();i.fftSize=8192,i.smoothingTimeConstant=.1,n.connect(i);let l=1/60,u=t/s,o=c=>{let y=l*c;y<u&&r.suspend(y).then(()=>{let b=new Float32Array(i.frequencyBinCount);i.getFloatFrequencyData(b),this.fftResults.push(b),o(c+1)}),c===1?r.startRendering():r.resume()};n.start(0),o(1),this.audio=e,this.context=r,this.analyser=i,this.sampleRate=s,this.audioBuffer=a}else{let t=new AudioContext,s=t.createMediaElementSource(e),r=t.createAnalyser();r.fftSize=8192,r.smoothingTimeConstant=.1,s.connect(r),r.connect(t.destination),this.audio=e,this.context=t,this.analyser=r,this.sampleRate=this.context.sampleRate,this.audioBuffer=null}}getFrequencies(e="frequency",a=-100,t=-30){let s=null;if(this.audioBuffer&&this.fftResults.length){let r=this.audio.currentTime/this.audio.duration,n=Math.min(r*this.fftResults.length|0,this.fftResults.length-1);s=this.fftResults[n]}return f.getFrequencies(this.analyser,this.sampleRate,s,e,a,t)}async resumeIfSuspended(){return this.context.state==="suspended"&&await this.context.resume(),!0}};globalThis.AudioAnalysis=g;var F=`
// StreamProcessor
...
`,T=new Blob([F],{type:"application/javascript"}),E=URL.createObjectURL(T),M=E;var S=class{constructor({sampleRate:e=44100}={}){this.scriptSrc=M,this.sampleRate=e,this.context=null,this.stream=null,this.analyser=null,this.trackSampleOffsets={},this.interruptedTrackIds={},this._audioProcessor=()=>{}}async connect(e){this.context=new AudioContext({sampleRate:this.sampleRate}),this._audioProcessor=e,this.context.state==="suspended"&&await this.context.resume();try{await this.context.audioWorklet.addModule(this.scriptSrc)}catch(t){throw console.error(t),new Error(`Could not add audioWorklet module: ${this.scriptSrc}`)}let a=this.context.createAnalyser();return a.fftSize=8192,a.smoothingTimeConstant=.1,this.analyser=a,this.stream||this._start(),!0}getFrequencies(e="frequency",a=-100,t=-30){if(!this.analyser)throw new Error("Not connected, please call .connect() first");return g.getFrequencies(this.analyser,this.sampleRate,null,e,a,t)}_start(){let e=new AudioWorkletNode(this.context,"stream_processor");return e.connect(this.context.destination),e.port.onmessage=a=>{let{event:t,data:s,timestamp_ms:r}=a.data;if(t==="audio")this._audioProcessor(s,r);else if(t==="stop")e.disconnect(),this.stream=null;else if(t==="offset"){let{requestId:n,trackId:i,offset:l,audio:u}=a.data,o=l/this.sampleRate;this.trackSampleOffsets[n]={trackId:i,offset:l,currentTime:o,audio:u}}else t==="log"&&console.log(s)},this.analyser.disconnect(),e.connect(this.analyser),this.stream=e,!0}configure(e){if(this.stream)this.stream.port.postMessage({event:"configure",config:e});else throw new Error("Not connected, please call .connect() first")}add16BitPCM(e,a="default"){if(typeof a!="string")throw new Error("trackId must be a string");if(this.interruptedTrackIds[a])return;this.stream||this._start();let t;if(e instanceof Int16Array)t=e;else if(e instanceof ArrayBuffer)t=new Int16Array(e);else throw new Error("argument must be Int16Array or ArrayBuffer");return this.stream.port.postMessage({event:"write",buffer:t,trackId:a}),t}async getTrackSampleOffset(e=!1){if(!this.stream)return null;let a=crypto.randomUUID();this.stream.port.postMessage({event:e?"interrupt":"offset",requestId:a});let t;for(;!t;)t=this.trackSampleOffsets[a],await new Promise(r=>setTimeout(()=>r(),1));let{trackId:s}=t;return e&&s&&(this.interruptedTrackIds[s]=!0),t}async interrupt(){let e=await this.getTrackSampleOffset(!0);return e?e.audio:null}};globalThis.WavStreamPlayer=S;var q=`
